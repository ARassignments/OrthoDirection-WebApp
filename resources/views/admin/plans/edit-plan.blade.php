...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Edit Pricing Plan</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('admin.plans') }}">Pricing Plans</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Edit Pricing Plan</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <div class="form-floating">
                            <input type="text" class="form-control text-capitalize" id="name" name="name" placeholder=""
                                autocomplete="off" required
                                onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123) || (event.charCode == 32)"
                                maxlength="50" value="{{ $plan['name'] }}">
                        ...
                        <div class="form-floating">
                            <select class="form-control text-capitalize" id="plan_type" name="plan_type" required>
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type }}" {{ old('plan_type', $plan->plan_type ?? '') == $type ? 'selected' : '' }} class="text-capitalize">
                                        {{ $type }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="monthly_price" name="monthly_price"
                                ...
                                maxlength="8" autocomplete="off" value="{{ $plan['monthly_price'] }}" disabled>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="yearly_price" name="yearly_price" placeholder=""
                                ...
                                autocomplete="off" value="{{ $plan['yearly_price'] }}" disabled>
                    ...
                    <div class="col-md-12">
                        <div class="form-floating">
                            <select class="form-control text-capitalize" id="plan_popular" name="plan_popular" required>
                                ...
                                <option value="1" class="text-capitalize bg-success-subtle text-success" {{ old('plan_popular', $plan->plan_popular ?? '') == 1 ? 'selected' : '' }}>
                                    Enable</option>
                                <option value="0" class="text-capitalize bg-danger-subtle text-danger" {{ old('plan_popular', $plan->plan_popular ?? '') == 0 ? 'selected' : '' }}>Disable
                                </option>
                            </select>
                        ...
                        <div class="d-md-flex align-items-center mt-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="status" id="status"
                                    value="{{ $plan['status'] }}" {{ $plan['status'] ? 'checked' : '' }}>
    ...
    <script>
        $(document).ready(function() {
            ...
            function addFeatureRow(tagValue = "") {
                ...
                newRow.innerHTML = `
                    <div class="col-md-6 col-6">
                        <div class="form-floating">
                            <input type="text" class="form-control text-capitalize" name="features[${rows.length}][name]" autocomplete="off" placeholder="Plan Feature"
                                value="${tagValue.name?tagValue.name:''}" required>
                    ...
                    <div class="col-md-5 col-3 pe-0">
                        <div class="form-floating">
                            <select class="form-control text-capitalize" name="features[${rows.length}][status]" required>
                                ...
                                <option value="1" class="text-capitalize bg-success-subtle text-success" ${tagValue.name?tagValue.status?'selected':'':'selected'}>Active</option>
                                <option value="0" class="text-capitalize bg-danger-subtle text-danger" ${tagValue.name?tagValue.status?'':'selected':''}>Deactive</option>
                            </select>
            ...
            let jsonString = @json($plan['features']);
            ...
            $('#planForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: `{{ url('admin/plans/planUpdate') }}/{{ $plan['id'] }}`,
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
